...
//func getSimulateFromSeedInput(tb testing.TB, w io.Writer, app *GoBitsong) (
...
//}
//
//func testAndRunTxs(app *GoBitsong) []simulation.WeightedOperation {
//	cdc := MakeCodec()
...
//}
//
//func invariants(app *GoBitsong) []sdk.Invariant {
...
//
//// Profile with:
//// /usr/local/go/bin/go test -benchmem -run=^$ github.com/cosmos/cosmos-sdk/GoBitsong -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
...
//		_ = os.RemoveAll(dir)
//	}()
//	app := NewBitsongApp(logger, db, nil, true, 0)
//
...
//	}()
//
//	app := NewBitsongApp(logger, db, nil, true, 0, fauxMerkleModeOpt)
//	require.Equal(t, "GoBitsong", app.Name())
//
//	// Run randomized simulation
...
//	}()
//
//	newApp := NewBitsongApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
...
//
//	newApp := NewBitsongApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
//	require.Equal(t, "GoBitsong", newApp.Name())
