...
var (
	...
	defaultAmount                  = defaultTokens.String() + assets.MicroBitSongDenom
...
func GenTxCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Long: fmt.Sprintf(`This command is an alias of the 'bitsongd tx create-validator' command'.
...
`, defaultAmount, defaultCommissionRate, defaultCommissionMaxRate, defaultCommissionMaxChangeRate, defaultMinSelfDelegation),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err = app.BitSongValidateGenesisState(genesisState); err != nil {
				return err
			}
