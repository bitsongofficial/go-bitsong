...
Example:
	...
	cmd.Flags().String(
		server.FlagMinGasPrices, fmt.Sprintf("0.00000008%s", assets.MicroBitSongDenom),
		"Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01ubtsg)",
	)
...
func initTestnet(config *tmconfig.Config, cdc *codec.Codec) error {
	...
	bitsongConfig.MinGasPrices = viper.GetString(server.FlagMinGasPrices)
	...
	for i := 0; i < numValidators; i++ {
		...
		accs = append(accs, app.GenesisAccount{
			...
			Coins: sdk.Coins{
				...
				sdk.NewCoin(assets.MicroBitSongDenom, accStakingTokens),
			},
		})
		...
		msg := staking.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(assets.MicroBitSongDenom, valTokens),
