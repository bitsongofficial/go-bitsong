...
`bitsongcli`是一个工具，使您能够与 Cosmos Hub 网络中的节点进行交互，无论您是否自己运行它。 让我们恰当的设置它。 要安装它，请按照[安装步骤](./ installation.md)进行安装。

### 配置 bitsongcli

设置`bitsongcli`的主要命令如下：
...
+ `cosmos` 
	+ 从通过`bitsongcli keys add`生成的账户私钥中产生
	+ 用于接收资金
...
+ `cosmosvalconspub`
	+ 在使用`bitsongd init`创建节点时生成
	+ 使用`bitsongd tendermint show-validator`获得该值
...
在广播交易时，`bitsongcli`接受`--broadcast-mode`标识。 这个标识的值可以是`sync`（默认值）、`async`或`block`，其中`sync`使客户端返回 CheckTx 响应，`async`使客户端立即返回，而`block`使得 客户端等待 tx 被提交（或超时）。
...
```bash
bitsongcli tx send ... --fees=50000uatom
```

或：

```bash
bitsongcli tx send ... --gas-prices=0.025uatom
```
...
```bash
bitsongcli query account <account_cosmos>
```
...
```bash
bitsongcli tx send <destination_cosmos> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> 
```
...
```bash
bitsongcli query account <account_cosmos>
bitsongcli query account <destination_cosmos>
```

你还可以使用`--block`标识查询在特定高度区块下你的余额：

```bash
bitsongcli query account <account_cosmos> --block=<block_height>
```

你可以通过在命令行中附加`--dry-run`标识来模拟交易而不实际广播它：

```bash
bitsongcli tx send <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> \
  --dry-run
```
...
```bash
bitsongcli query staking validator <account_cosmosval>
```
...
其中`[name]`是初始化`bitsongd`时指定的键的名称。
...
```bash
bitsongcli query staking unbonding-delegations <account_cosmos>
```

此外，你可以从特定验证人获取所有unbonding-delegation：

```bash
bitsongcli query staking unbonding-delegations-from <account_cosmosval>
```
...
```bash
bitsongcli query staking redelegations <account_cosmos>
```

此外，你可以查询某个特定验证人的所有转出的重新绑定：

```bash
bitsongcli query staking redelegations-from <account_cosmosval>
```
...
```bash
bitsongcli query delegations-to <account_cosmosval>
```
...
```bash
bitsongcli tx gov deposit <proposal_id> "10000000uatom" \
  --from=<name> \
  --chain-id=<chain_id>
```
...
可以通过完全命令生成主流的UNIX shell解释器（如`Bash`和`Zsh`）的`completion`命令，该命令可用于`bitsongd`和`bitsongcli`。
