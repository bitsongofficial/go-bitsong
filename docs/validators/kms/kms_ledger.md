...
- Adjust `chain-id` to match your `.bitsongd/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `bitsongd`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell bitsongd the key that we've got in the previous section.

```bash
bitsongd gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `bitsongd`. You should see that the KMS connects and receives a signature request.
