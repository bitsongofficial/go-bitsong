...
func CosmWasmIntegration(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, user ibc.Wallet) (contractAddr string) {
...
// 	_, senderContractAddr := helpers.SetupContract(t, ctx, chain, user.KeyName(), "contracts/cw721_base.wasm.gz", fmt.Sprintf(`{"name":"Eret", "symbol":"skeret-test-#00001", "minter":"%s"}`, user.FormattedAddress()))
