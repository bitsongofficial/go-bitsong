...
# bitsongd q bank balances cosmos1mzgucqnfr2l8cj5apvdpllhzt4zeuh2cshz5xu --home ./data/localnet --output json | jq
echo "-> Checking user1 account balances..."
$BINARY q bank balances $USER_1_ADDRESS \
--home $CHAIN_DIR/$CHAIN_ID \
--output json | jq

# bitsongd q bank balances cosmos185fflsvwrz0cx46w6qada7mdy92m6kx4gqx0ny --home ./data/localnet --output json | jq
echo "-> Checking user2 account balances..."
$BINARY q bank balances $USER_2_ADDRESS \
--home $CHAIN_DIR/$CHAIN_ID \
--output json | jq

# bitsongd q liquidity batch 1 --home ./data/localnet --chain-id localnet --output json | jq
...
# bitsongd tx liquidity deposit 1 100000ubtsg,200000token --home ./data/localnet --chain-id localnet --from user1 --keyring-backend test --yes
echo "-> Depositing coins to the liquidity pool 1..."
$BINARY tx liquidity deposit 1 100000ubtsg,200000token \
--home $CHAIN_DIR/$CHAIN_ID \
--chain-id $CHAIN_ID \
--from user1 \
...
# bitsongd q liquidity deposits 1 --home ./data/localnet --output json | jq
...
# Ensure the existence of the liquidity pool. 
# If there is no liquidity pool created then use create-pool script to create liquidity pool.
# bitsongd tx liquidity deposit 2 100000ubtsg,200000token --home ./data/localnet --chain-id localnet --from user2 --keyring-backend test --yes
echo "-> Depositing coins to the liquidity pool 2..."
$BINARY tx liquidity deposit 2 100000ubtsg,200000atom \
--home $CHAIN_DIR/$CHAIN_ID \
--chain-id $CHAIN_ID \
--from user2 \
--keyring-backend test \
...
# bitsongd q liquidity deposits 2 --home ./data/localnet --output json | jq
echo "-> Querying liquidity deposits..."
