...
# bitsongd tx liquidity swap 1 1 79000ubtsg token 0.99 0.003 --home ./data/localnet --chain-id localnet --from user1 --keyring-backend test --yes
echo "-> Swapping coins from the liquidity pool 1..."
$BINARY tx liquidity swap 1 1 79000ubtsg token 0.99 0.003 \
--home $CHAIN_DIR/$CHAIN_ID \
--chain-id $CHAIN_ID \
--from user1 \
...
# bitsongd q liquidity swaps 1 --home ./data/localnet --output json | jq
...
# Ensure the existence of the liquidity pool. 
# If there is no liquidity pool created then use create-pool script to create liquidity pool.
# bitsongd tx liquidity swap 2 100000ubtsg,200000token --home ./data/localnet --chain-id localnet --from user2 --keyring-backend test --yes
echo "-> Swapping coins from the liquidity pool 2..."
$BINARY tx liquidity swap 2 1 79000ubtsg atom 0.99 0.003 \
--home $CHAIN_DIR/$CHAIN_ID \
--chain-id $CHAIN_ID \
--from user2 \
--keyring-backend test \
