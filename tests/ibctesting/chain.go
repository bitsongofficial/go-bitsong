...
// GetBitsongApp returns the current chain's app as an BitsongApp
func (chain *TestChain) GetBitsongApp() *app.BitsongApp {
	v, _ := chain.App.(*app.BitsongApp)
	return v
}
...
func (chain *TestChain) SendMsgsFromPrivKeys(privKeys []cryptotypes.PrivKey, msgs ...sdk.Msg) (*abci.ExecTxResult, error) {
	...
	for i, msg := range msgs {
		...
		account := chain.GetBitsongApp().AppKeepers.AccountKeeper.GetAccount(chain.GetContext(), signerAcc)
	...
	for _, msg := range msgs {
		...
		signer := signers[0]
		account := chain.GetBitsongApp().AppKeepers.AccountKeeper.GetAccount(chain.GetContext(), signer)
