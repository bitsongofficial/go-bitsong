...
func GetIBCHooksUserAddress(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, channel, uaddr string) string {
	// bitsongd q ibchooks wasm-sender channel-0 "terp1hj5fveer5cjtn4wd6wstzugjfdxzl0xppxm7xs" --node http://localhost:26657
	cmd := []string{
		"bitsongd", "query", "ibchooks", "wasm-sender", channel, uaddr,
