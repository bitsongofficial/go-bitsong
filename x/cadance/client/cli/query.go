...
func GetCmdShowContracts() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "contracts",
		Short: "Show addresses of all current cadance contract s",
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			res, err := queryClient.CadanceContracts(cmd.Context(), &types.QueryCadanceContracts{
				Pagination: pageReq,
			})
			if err != nil {
				return err
...
func GetCmdShowContract() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			req := &types.QueryCadanceContract{
				ContractAddress: args[0],
			}

			res, err := queryClient.CadanceContract(cmd.Context(), req)
			if err != nil {
				return err
			}
