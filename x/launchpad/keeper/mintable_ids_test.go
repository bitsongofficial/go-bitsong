...
func (suite *KeeperTestSuite) TestMintableMetadataIdsGetSetDelete() {
	...
	metadataIds := suite.app.LaunchPadKeeper.GetMintableMetadataIds(suite.ctx, 1)
	suite.Require().Len(metadataIds, 0)

	// set mintable metadata ids and check
	suite.app.LaunchPadKeeper.SetMintableMetadataIds(suite.ctx, 1, []uint64{1, 2, 3, 4})
	metadataIds = suite.app.LaunchPadKeeper.GetMintableMetadataIds(suite.ctx, 1)
	suite.Require().Len(metadataIds, 4)

	// delete mintable metadata ids and check
	suite.app.LaunchPadKeeper.DeleteMintableMetadataIds(suite.ctx, 1)
	metadataIds = suite.app.LaunchPadKeeper.GetMintableMetadataIds(suite.ctx, 1)
	suite.Require().Len(metadataIds, 0)
}
...
func (suite *KeeperTestSuite) TestTakeOutFirstMintableMetadataId() {
	...
	metadataId := suite.app.LaunchPadKeeper.TakeOutFirstMintableMetadataId(suite.ctx, 1)
...
func (suite *KeeperTestSuite) TestTakeOutRandomMintableMetadataId() {
	...
	metadataId := suite.app.LaunchPadKeeper.TakeOutRandomMintableMetadataId(suite.ctx, 1, 4)
...
func (suite *KeeperTestSuite) TestShuffleMintableMetadataIds() {
	...
	suite.app.LaunchPadKeeper.ShuffleMintableMetadataIds(suite.ctx, 1)
	metadataIds = suite.app.LaunchPadKeeper.GetMintableMetadataIds(suite.ctx, 1)
	suite.Require().Len(metadataIds, 4)
}
