...
import (
	...
	nftcli "github.com/bitsongofficial/bitsong/x/nft/client/cli"
	nfttestutil "github.com/bitsongofficial/bitsong/x/nft/client/testutil"
...
func (s *IntegrationTestSuite) TestNft() {
	...
	url := fmt.Sprintf("%s/bitsong/nft/denoms/%s", baseURL, denomID)
	...
	url = fmt.Sprintf("%s/bitsong/nft/denoms/name/%s", baseURL, denomName)
	resp, err = rest.GetRequest(url)
	respType = proto.Message(&nfttypes.QueryDenomResponse{})
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.JSONMarshaler.UnmarshalJSON(resp, respType))
	...
	url = fmt.Sprintf("%s/bitsong/nft/denoms", baseURL)
	resp, err = rest.GetRequest(url)
	...
	url = fmt.Sprintf("%s/bitsong/nft/collections/%s/supply", baseURL, denomID)
	resp, err = rest.GetRequest(url)
	...
	//------test GetCmdQueryNFT()-------------
	url = fmt.Sprintf("%s/bitsong/nft/nfts/%s/%s", baseURL, denomID, tokenID)
	resp, err = rest.GetRequest(url)
	...
	url = fmt.Sprintf("%s/bitsong/nft/nfts?owner=%s", baseURL, from.String())
	resp, err = rest.GetRequest(url)
	...
	url = fmt.Sprintf("%s/bitsong/nft/collections/%s", baseURL, denomID)
	resp, err = rest.GetRequest(url)
